...
FROM sucursal
NATURAL JOIN((SELECT num 
	FROM producto WHERE nombre='Pizza napolitana') 
	INTERSECT (SELECT num 
	FROM producto WHERE nombre='Pizza napolitana especial'))
...
NATURAL JOIN ((SELECT num 
	FROM sucursal 
	WHERE provincia='San Juan')UNION
	(SELECT num 
	FROM sucursal 
	WHERE provincia='Mendoza')UNION
	(SELECT num 
	FROM sucursal 
...
SELECT s.num, sum(d.precio*d.cantidad)
FROM sucursal s
