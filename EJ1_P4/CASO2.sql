...
--1.
SELECT first_name AS Nombre FROM Employees
SELECT last_name AS Apellido FROM Employees
...
(SELECT last_name,commission_pct FROM EMPLOYEES 
	WHERE commission_pct>0.25) ORDER BY last_name DESC

--4. Obtenga la cantidad de empleados que trabajan en el departamento 100.
SELECT count(*) FROM employees NATURAL JOIN 
...
SELECT * FROM departments WHERE department_id=10
	UNION SELECT * FROM departments WHERE department_id=70 

--6.  Obtenga el apellido (last_name) de los empleados junto al nombre 
	--del departamento (department_name) donde trabajan.
SELECT EMPLOYEES.last_name, DEPARTMENTS.department_name 
	FROM EMPLOYEES JOIN DEPARTMENTS ON EMPLOYEES.department_id=DEPARTMENTS.department_id

--7. Obtenga el apellido (last_name) de los empleados de los departamentos 
	--de Finanzas (Finance) y/o Transporte (Shipping).
SELECT EMPLOYEES.last_name FROM EMPLOYEES 
	JOIN DEPARTMENTS ON EMPLOYEES.department_id=DEPARTMENTS.department_id
	WHERE DEPARTMENTS.department_name IN ('Finance', 'Shipping')
...
--9.
SELECT DISTINCT departments.department_id, jobs.job_title
	FROM employees
	JOIN jobs ON employees.job_id = jobs.job_id
	JOIN departments ON employees.department_id = departments.department_id
	WHERE jobs.job_title IN ('Stock Clerk', 'Shipping Clerk')
...
--13.Obtenga el nombre del/de los empleados que tienen el sueldo 
--máximo del departamento 110. Ordene el listado en forma ascendente por salario
SELECT employees.first_name FROM employees WHERE department_id= 110 
and salary= (SELECT max(salary) FROM employees WHERE department_id= 110)
ORDER BY salary ASC

--14.  Obtenga todos los datos de los empleados que no sean supervisores (o directores)
-- NOTA: En la tabla departments el atributo manager_id tiene 
--la identificación de los supervisores de cada dpto.
